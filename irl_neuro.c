...
double *theta; /* N_FEATURE out */
{
    ...
    inpData=(double **)comMalloc(sizeof(double *)*N_FEATURE);
    outData=(double **)comMalloc(sizeof(double *)*N_FEATURE);

    for(i=0;i<N_FEATURE;i++) {
      inpData[i]=(double *)comMalloc(sizeof(double)*(1+N_FEATURE));
      outData[i]=(double *)comMalloc(sizeof(double)*2);
    }
...
#endif
    /* make neuro model */
    pNode=IRLneuroMake(TAG_GRAD,nFeature,nFeature+1,nFeature,1,nStep,nHidden,&mj);
    if(!pNode) {
        fprintf(stderr,"cannot make neuro structure\n");
        return(-9);
    }
    ...
    for(L=0;L<n_epoch;L++) {
        ...
        for(i=0;i<nFeature;i++) {
          ...
          inpData[i][0]= grad[i];
#else
          inpData[i][0]=theta[i] + ALPHA/10*grad[i];
#endif
          for(j=0;j<nFeature;j++) {
            if(i == j) inpData[i][1+j]=1;  
            else       inpData[i][1+j]=0;
          }
        }
        /* backward */
        IRLbackward(TAG_VALUE,pNode,nFeature,nFeature+1,inpData,outData,nFeature,1,nStep,mj,100,0.001,FUNC_RELU);
        //IRLneuro(TAG_GRAD,inpData,N_FEATURE,N_FEATURE+1,N_FEATURE,1,outData,4,100,0.001,64,FUNC_RELU);
        for(i=0;i<nFeature;i++) {
          //theta[i] = outData[i][0];
          theta[i] = outData[i][0];
        }
...
char *argv[];
{
    ...
    max_ent_irl(feature_matrix, &trans, nTrajs, &trajs, 200, res);
